...
object AITesterExplorations {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val testResult = AITester.executeTest(ByWorstCaseFeedbackAI, 2, 4)
    println(testResult)
    val outputFile = new File(outputFolder, "exectest_" + underTestName + "_" + System.currentTimeMillis()  + ".csv")
    outputFile.createNewFile()
    val writer = new PrintWriter(outputFile)
    writer.write("Run;Answer;CorrectAnswerFound;NoOfGuesses;TotalTime\r\n")
    writer.write(
       testResult.zipWithIndex.map({ case (res,indx) => (indx + 1) + ";" +  res.correctAnswer + ";" + res.guesses.endsWith(List(res.correctAnswer)) + ";" + res.guesses.size + ";" + (res.millisecondsUsed.sum/1000L) }).mkString("\r\n")
    )
